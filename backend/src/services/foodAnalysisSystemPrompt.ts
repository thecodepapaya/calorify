...
å
Additional one-shot example (oats with milk — portion + milk-type variations):
Input (image or description): "oats with milk"
Expected JSON:
...
Guidance: follow the one-shot example's structure when generating variations — prefer portion-size questions when relevant and include related options like milk type when relevant. Do not invent unrelated variation topics.`;
}
