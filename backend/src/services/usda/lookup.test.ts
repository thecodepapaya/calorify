...
await mock.module('../infrastructure/database.js', {
...
test('canonicalizeUsdaProposal evaluates canonical and every alias before selecting', async () => {
  clearUsdaLookupCache();
  mockQuery.mock.resetCalls();
  const queried: string[] = [];
  mockQuery.mock.mockImplementation(async (sql: string, params?: unknown[]) => {
    const term = String(params?.[0] ?? '');
    queried.push(term);
    if (!sql.includes('similarity') && term === 'garbanzo beans') {
      return { rows: [{ ...LENTILS_ROW, description: 'Chickpeas, cooked', normalized_name: 'garbanzo beans' }] };
    }
    return { rows: [] };
  });
  const result = await canonicalizeUsdaProposal(
    'chickpeas',
    ['garbanzo beans', 'bengal gram'],
    ['boiled']
  );
  assert.equal(result.row?.normalized_name, 'garbanzo beans');
  assert.deepEqual(new Set(queried), new Set(['chickpeas', 'garbanzo beans', 'bengal gram']));
});
