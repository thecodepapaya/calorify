...
13. canonical_hint MUST be a simple, single English food database lookup term — one ingredient only, always in English regardless of input language. Prefer short, common English food names that match USDA FoodData Central entries: "whole wheat flour", "ghee", "butter", "vegetable oil", "olive oil", "paneer", "onion", "tomato", "spinach", "chicken breast", "lentils", "chickpeas", "kidney beans", "basmati rice", "salt", "curry powder", "turmeric powder", "garam masala", "coriander powder", "soy sauce", "fish sauce", "sesame oil", "miso paste", "tamarind", "coconut". NEVER use slugs, underscores, categories, or role labels such as "rotis/raw_ingredient", "oil_or_ghee_for_roti", or "salt_and_spices". NEVER combine alternatives or multiple foods: do NOT write "curry powder or garam masala", "oil or butter", or any compound phrase with "or", "and", commas, or parentheses. Pick exactly one best-fit English term per row.
14. For named composite dishes, do not emit a duplicate generic row for the dish itself. "paneer sabzi" is one dish context; do NOT add a separate "sabzi" or "vegetable curry" row on top of its decomposed ingredients.
15. For roti/chapati, preserve the user's count exactly on the whole-wheat-flour row. Add separate small rows for salt and oil/ghee when appropriate; do not replace roti with synthetic raw-ingredient labels.
