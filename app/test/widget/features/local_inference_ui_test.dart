...
void main() {
  ...
  testWidgets('result details distinguish local, edited, and remote stages', (
    tester,
  ) async {
    final pipelineContext = MealAnalysisPipelineSessionContext(
      result: PipelineResultData(
        ...
        macros: PipelineMacros(calories: 280),
        ingredients: [
        ...
        receipt: MealAnalysisReceipt(
          ...
          nutritionOrigin: NutritionOrigin.NUTRITION_ORIGIN_REMOTE_USDA,
          calculationOrigin:
              CalculationOrigin.CALCULATION_ORIGIN_SERVER_DETERMINISTIC,
          calculationVersion: 'nutrition-engine-v2',
          fallbackReason:
    ...
    expect(find.text('Ingredients interpreted on this device'), findsWidgets);
    expect(find.text('Ingredients reviewed or edited by you'), findsOneWidget);
    expect(
      find.text('Nutrition fetched from USDA through Calorify'),
      findsOneWidget,
    );
    expect(
      find.text('Calories and macros calculated by Calorify'),
      findsOneWidget,
    );
  });
